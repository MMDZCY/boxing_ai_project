...
STANDARD_ACTION_PATHS = {
    0: r"D:\boxing_ai_project\standard_action\standard_jab.npy",      # 刺拳
    1: r"D:\boxing_ai_project\standard_action\standard_straight.npy",  # 直拳
    2: r"D:\boxing_ai_project\standard_action\standard_hook.npy",      # 勾拳
    3: r"D:\boxing_ai_project\standard_action\standard_swing.npy"      # 摆拳
}
...
def main():
    ...
    last_match_idx = 0
    action_switch_cooldown = 0  # 动作切换冷却，防止频繁切换
    ...
    while True:
                ...
                if pred_class is not None:
                    ...
                    if action_switch_cooldown == 0 and conf > 0.7 and pred_class in standard_actions:
                        ...
                        if current_std_data['name'] != standard_actions[pred_class]['name']:
                            ...
                            last_match_idx = 0
                            action_switch_cooldown = 30  # 冷却30帧，防止频繁切换
            ...
            if cooldown_counter > 0:
                ...
                if len(wrist_history) == WINDOW_SIZE:
                    ...
                    if movement_dist > PUNCH_SPEED_THRESHOLD and current_elbow > 100:
                        ...
                        print(f"🎯 第{len(score_history)}拳 | {current_action} | 得分：{total:.1f}/80 | {last_suggestion}")
        ...
        # UI界面
        cv2.rectangle(display_frame, (10, 10), (420, 300), COLOR_BLACK, -1)
        cv2.rectangle(display_frame, (10, 10), (420, 300), COLOR_WHITE, 2)
            ...
            if analysis and analysis['top_error']:
                ...
                if error_info['error_rate'] > 0.5:
                    display_frame = put_chinese_text(display_frame, 
                        ...
                        (20, 215), font_size=14, color=COLOR_ERROR)
        
        if 'match_info' in locals():
            display_frame = put_chinese_text(display_frame, match_info, (20, 250), font_size=14, 
        ...
        if last_score is not None:
            ...
            display_frame = put_chinese_text(display_frame, f"动作：{current_action}", (FRAME_WIDTH-400, FRAME_HEIGHT-155), font_size=16, color=action_color)
        ...
        if key == ord('r'):
            if len(error_analyzer.punch_history) >= 5:
                ...
                print("⚠️  数据不足，至少需要5次出拳才能生成报告")

        cv2.imshow("🥊 拳击动作AI训练系统 (错误识别版)", display_frame)
